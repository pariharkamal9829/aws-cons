...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Only allow local dev origins and trusted domains
    ...
    devServer.app.post("/edit-file", (req, res) => {
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            // Commit changes with the current user's git config
            try {
              execSync(`git add "${targetFile}"`);
              execSync(`git commit -m "visual_edit_${timestamp}"`);
            } catch (e) {
              // If git fails (not a git repo or misconfigured), ignore and continue
            }
          } catch (gitError) {
